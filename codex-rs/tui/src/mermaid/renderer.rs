...
// Adapted from upstream commit 75810042ca2762aa0b0fa17864f3f68823ccbea5.
// Modified for Codex: explicit fallback outcome, strict parsing of incomplete
// diagrams, and terminal-only fallback guidance. See README.md for provenance.
...
impl Graph {
    fn node_index(&mut self, id: &str, label: Option<&str>, shape: Shape) -> Option<usize> {
        if let Some(&i) = self.index.get(id) {
            // A later declaration inside a group must not inherit an earlier outer scope.
            if self.cur_group.is_some() && self.cur_group != self.node_group[i] {
                return None;
            }
...
fn parse_graph(src: &str) -> Option<Graph> {
    ...
    for st in &statements[1..] {
        ...
        match first_word.to_ascii_lowercase().as_str() {
            ...
            "classdef" | "class" | "style" | "linkstyle" | "click" => continue,
            // A scoped direction changes layout semantics; this renderer uses one global direction.
...
fn parse_subgraph_decl(rest: &str) -> Option<(String, String)> {
    if rest.is_empty() {
        return None;
    }
    if let Some(q) = rest.strip_prefix('"') {
        let label = q.strip_suffix('"')?;
        if label.is_empty() || label.contains('"') {
            return None;
        }
        return Some((label.to_string(), decode_html_entities(label)));
    }
    if let Some(open) = rest.find('[') {
        let id = rest[..open].trim();
        let label = rest[open + 1..].strip_suffix(']')?.trim();
        if id.is_empty() || !id.chars().all(is_id_char) || label.is_empty() {
            return None;
        }
        if label.starts_with('"') && !label.ends_with('"') {
            return None;
        }
        return Some((id.to_string(), clean_label(label)));
    }
    if rest.contains(['[', ']', '"']) {
        return None;
    }
...
fn parse_statement(st: &str, graph: &mut Graph) -> Option<()> {
    let chars: Vec<char> = st.chars().collect();
    let mut i = 0;

    let (mut prev, ni) = parse_node_group(&chars, i, graph)?;
    i = ni;
    ...
    loop {
        ...
        let (left, right, line, label, ni) = parse_link(&chars, i)?;
        i = skip_spaces(&chars, ni);
        let (next, ni) = parse_node_group(&chars, i, graph)?;
        i = ni;
        for &f in &prev {
            for &t in &next {
...
fn read_shape(
    ...
    closer: &str,
    shape: Shape,
) -> Option<(Option<Shape>, Option<String>, usize)> {
    ...
    while i < chars.len() {
        ...
        if !in_quotes && chars[i..].starts_with(closer.as_slice()) {
            ...
            return Some((Some(shape), Some(label), i + closer.len()));
        }
        text.push(c);
        i += 1;
    }
    ...
    for st in &statements[1..] {
        ...
        match first.to_ascii_lowercase().as_str() {
            "direction" => {
                ...
                if st.split_whitespace().count() != 2 {
                    return None;
                }
            }
            "state" => {
                // Composite regions need separately scoped states and initial/final markers.
                if st.ends_with('{') {
                    return None;
                }
...
fn parse_class(src: &str) -> Option<(Graph, Vec<ClassInfo>)> {
    ...
    for st in &statements[1..] {
        ...
        match first.to_ascii_lowercase().as_str() {
            ...
            "note" | "callback" | "click" | "link" | "style" | "cssclass" | "classdef" => continue,
    ...
    if graph.nodes.is_empty() || cur_class.is_some() {
        return None;
    }
    sync_infos(&graph, &mut infos);
    Some((graph, infos))
...
fn parse_er(src: &str) -> Option<(Graph, Vec<ClassInfo>)> {
    ...
    if graph.nodes.is_empty() || cur_entity.is_some() {
        return None;
    }
    sync_infos(&graph, &mut infos);
    Some((graph, infos))
    ...
    let box_h: Vec<usize> = (0..n)
        .map(|i| match &extras[i] {
            ...
            NodeExtra::Compartments(sections) => {
                ...
                sections.iter().map(std::vec::Vec::len).sum::<usize>()
...
fn parse_sequence(src: &str) -> Option<Sequence> {
    ...
    if seq.labels.is_empty() || !blocks.is_empty() {
        return None;
    }
    Some(seq)
}
...
fn parse_seq_message(
    ...
    if from_id.is_empty() || from_id.contains(char::is_whitespace) {
        return None;
    }
    ...
    if to_id.is_empty() || to_id.contains(char::is_whitespace) {
        return None;
    }
...
fn layout_sequence(
    ...
    let item_text_w = |text: &Option<String>| {
        text.as_deref()
...
const TOO_WIDE_HINT: &str =
    "This diagram is too wide to display here. Widen the terminal to view it.";
...
mod tests {
    ...
    fn too_wide_fallback_appends_hint_below_box() {
        ...
        assert!(
            joined.contains("Widen the terminal"),
            "note explains how to fit the diagram:\n{joined}"
        );
    ...
    fn shared_target_back_edges_share_one_lane() {
        ...
        assert_eq!(
            two.lines().map(unicode_width::UnicodeWidthStr::width).max(),
            one.lines().map(unicode_width::UnicodeWidthStr::width).max(),
    ...
    fn subgraph_ambiguous_redeclarations_preserve_source() {
        for source in [
            ...
            assert!(
                art.is_fallback,
                "must not replace a child endpoint with its group: {source}"
            );
            let output = art.plain_lines.join("\n");
            for line in source.lines() {
                assert!(output.contains(line), "original endpoint omitted: {line}");
            }
        }
    }
    ...
    #[test]
    fn state_composite_and_concurrent_regions_preserve_source() {
        for source in [
            "stateDiagram-v2\n [*] --> Active\n state Active {\n [*] --> Waiting\n Waiting --> [*]\n }\n Active --> [*]",
            "stateDiagram-v2\n state Active {\n [*] --> A\n --\n [*] --> B\n }",
            "stateDiagram-v2\n [*] --> A\n --\n [*] --> B",
            ...
            assert!(
                art.is_fallback,
                "regions must not share state markers: {source}"
            );
            let output = art.plain_lines.join("\n");
            for line in source.lines() {
                assert!(output.contains(line), "source line omitted: {line}");
            }
        }
    }

    #[test]
    fn state_notes_preserve_source() {
        for source in [
            "stateDiagram-v2\n A --> B\n note right of A: Check permission before continuing",
            "stateDiagram-v2\n A --> B\n note left of B\n Wait for confirmation\n end note",
        ] {
            let art = render(source, &styles(), None).unwrap();
            assert!(art.is_fallback, "state notes must remain visible: {source}");
            let output = art.plain_lines.join("\n");
            for line in source.lines() {
                assert!(output.contains(line), "source line omitted: {line}");
            }
    ...
    #[test]
    fn malformed_flowchart_keeps_source_instead_of_partial_diagram() {
        for source in [
            "flowchart TD\n A --> B\n this is not a node declaration",
            "flowchart TD\n A -->",
            "flowchart TD\n A[unclosed --> B",
            "flowchart TD\n A[\"unclosed] --> B",
            "flowchart TD\n A --> B unexpected tail",
            ...
            let art = render(source, &styles(), None).unwrap();
            assert!(art.is_fallback, "must not show a partial diagram: {source}");
            assert!(
                art.plain_lines
                    .join("\n")
                    .contains(source.lines().next().unwrap())
            );
        }
    }

    #[test]
    fn incomplete_sibling_diagrams_preserve_source() {
        for source in [
            "sequenceDiagram\n A->>B: hi\n loop retry\n B->>A: retry",
            "sequenceDiagram\n A->>B: hi\n end",
            "sequenceDiagram\n A->>B: hi\n else orphan",
            "sequenceDiagram\n A->>B: hi\n note over A,B,C: incomplete scope",
            "classDiagram\n class A {\n +int value",
            "classDiagram\n direction sideways\n A --> B",
            "classDiagram\n namespace X {\n class A\n }",
            "erDiagram\n A {\n int id",
            ...
            "stateDiagram-v2\n A --> B\n }",
            "stateDiagram-v2\n A --> B\n note left of A\n missing end",
            "stateDiagram-unknown\n A --> B",
        ] {
            let art = render(source, &styles(), None).unwrap();
            assert!(
                art.is_fallback,
                "must preserve incomplete structure: {source}"
            );
        }
    }

    #[test]
    fn fallback_outcome_distinguishes_diagrams_for_the_markdown_adapter() {
        for source in [
            "flowchart TD\n A --> B",
            "flowchart LR\n subgraph g[Group]\n A --> B\n end",
            "sequenceDiagram\n A->>B: hello",
            ...
            let art = render(source, &styles(), None).unwrap();
            assert!(!art.is_fallback, "valid supported diagram: {source}");
        }
        let source = "flowchart LR\n A[Start] --> B[Finish]";
        assert!(render(source, &styles(), Some(8)).unwrap().is_fallback);
        assert!(
            render("pie\n \"A\" : 1", &styles(), None)
                .unwrap()
                .is_fallback
